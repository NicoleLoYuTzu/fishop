...
class FishSellerAddTodayAdapterItem() :
    ListAdapter<Category, RecyclerView.ViewHolder>(DiffCallback) {


    class RecordHolder(private var binding: FragmentFishSellerAddTodayItemBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun bind(category: Category) {
            binding.category = category
            binding.textViewCategory.text = category.categoryName
            binding.recyclerView.adapter= FishSellerAddTodayAdapterItemItem()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ITEM_VIEW_TYPE_RECORD -> RecordHolder(
                FragmentFishSellerAddTodayItemBinding.inflate(
